...
describe('themeCssVariables', () => {
  ...
  describe('themeCssVariablesText', () => {
    it('wraps the variables in a rule that outranks the generated defaults', () => {
      const cssText = themeCssVariables.themeCssVariablesText();
      expect(cssText).toMatch(/^html:root\s*\{/);
  ...
  describe('helper-styles.scss', () => {
    it('ships the theme variable defaults, so a `var()` has a value before the JS runs', () => {
      // the `themeDefaults` import is what makes this true, and nothing else fails
      // without it, so this is the only thing holding that import in place
      const css = require('node-sass')
        .renderSync({ file: path.resolve(__dirname, '../helper-styles.scss') })
        .css.toString();
      const emitted = Object.keys(themeCssVariables.generateThemeCssVariables());
      expect(emitted).not.toHaveLength(0);
      expect(emitted.filter(name => !css.includes(`${name}:`))).toEqual([]);
    });
  });
  ...
  describe('globals.scss', () => {
    it('is imported in common.scss', () => {
      const css = require('node-sass')
        .renderSync({
          file: path.resolve(__dirname, '../common.scss'),
          // node-sass doesn't resolve webpack's `~` prefix for node_modules imports
          importer: url =>
            url.startsWith('~')
        ...
        .css.toString();
      expect(css).toContain('normalize.css'); // purecss base
