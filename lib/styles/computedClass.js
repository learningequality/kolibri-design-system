...
function generateClassName(value, component) {
  ...
  if (!__classNameCache[cacheKey]) {
    ...
    const output = nuxtServerSideRendering ? '' : css(ids.map(id => styleSheet[id]));
    __classNameCache[cacheKey] = output;
  }
 ...
 * release. Use theme CSS variables (`var(--tokens-*)`) in a `<style>` block, or `v-bind()`
 * for a value computed at runtime. See https://design-system.learningequality.org/colors#computed-classes
 */
