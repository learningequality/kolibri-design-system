...
describe.visual('KSelect Visual Tests', () => {
  ...
  it('renders KSelect with multiple options selected and dropdown open', async () => {
    await renderComponentForVisualTest('KSelect', {
      label: 'Select options',
      options,
      multiple: true,
    ...
    await click('.ui-select-label');
    await takeSnapshot('KSelect multiple option selected with dropdown open', snapshotOptions);
  });
});
