...
export default {
  ...
  computed: {
    // For when we want to force a renderer to use time-based progress (e.g. instead of %)
    forcedTimeBasedProgress() {
      return this.options.force_time_based_progress;
    },
    // Uses clock-time to track time so that all content types can be tracked the same way
    durationBasedProgress() {
      return this.timeSpent / (this.duration || this.durationDefault);
    },
    defaultItemPreset() {
