...
function getKDateRangeElements(wrapper) {
  return {
    startDate: () => wrapper.find('[data-testid="startDate"]'),
    endDate: () => wrapper.find('[data-testid="endDate"]'),
    kModal: () => wrapper.find('[data-testid="dateRangeModal"]'),
  };
}
...
function getKDateCalendarProps() {
  return {
    firstAllowedDate: new Date(2022, 0, 1),
...
function makeKDateCalendarWrapper(kDateCalendarProps = {}) {
  return mount(KDateCalendar, {
    propsData: { ...getKDateCalendarProps(), ...kDateCalendarProps },
  });
}

function getKDateCalendarElements(wrapper) {
  return {
    previousMonthDisplay: () => wrapper.find('[data-testid="previousMonth"]'),
    currentMonthDisplay: () => wrapper.find('[data-testid="currentMonth"]'),
  };
}
