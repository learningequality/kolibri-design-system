...
export default function useMultiSelectDropdown(
  props,
  { containerEl, searchText, normalizedOptions, getOptionValue },
) {
  ...
  function openDropdown() {
    ...
    isOpen.value = true;
    if (!props.multiple && props.value != null && props.value !== '' && !searchText.value) {
      const opt = normalizedOptions.value.find(o => getOptionValue(o) === props.value);
      if (opt) {
        searchText.value = opt[props.itemText];
      }
    }
  }
