...
describe('useMultiSelectList', () => {
  ...
  it('passes through object options unchanged', () => {
    ...
    expect(normalizedOptions.value[0]).toEqual({ id: '1', name: 'Option One', tags: ['red', 'first'] });
  });
  ...
  it('filters options by itemText (case-insensitive)', () => {
    ...
    expect(displayedOptions.value).toEqual([{ id: '1', name: 'Option One', tags: ['red', 'first'] }]);
  });
  ...
  it('filters by a string-valued searchKey', () => {
    ...
    const { displayedOptions } = useMultiSelectList(props, emit, { searchText });
    searchText.value = '2';
    expect(displayedOptions.value).toEqual([{ id: '2', name: 'Option Two', tags: ['blue', 'second'] }]);
  });
  ...
  it('maps selected value to full option object in single mode', () => {
    ...
    props.value = '2';
    const { selectedOptionsData } = useMultiSelectList(props, emit, { searchText });
    expect(selectedOptionsData.value).toEqual([{ id: '2', name: 'Option Two', tags: ['blue', 'second'] }]);
  });
  ...
  it('toggleOption adds an option to selection when not already selected', () => {
    props.value = ['1'];
    const { toggleOption } = useMultiSelectList(props, emit, { searchText });
    toggleOption({ id: '2', name: 'Option Two', tags: ['blue', 'second'] });
    expect(emit).toHaveBeenCalledWith('input', ['1', '2']);
  });

  it('toggleOption removes an option from selection when already selected', () => {
    props.value = ['1', '2'];
    const { toggleOption } = useMultiSelectList(props, emit, { searchText });
    toggleOption({ id: '1', name: 'Option One', tags: ['red', 'first'] });
    expect(emit).toHaveBeenCalledWith('input', ['2']);
  });

  it('toggleOption sets value directly in single mode', () => {
    props.multiple = false;
    props.value = null;
    const { toggleOption } = useMultiSelectList(props, emit, { searchText });
    toggleOption({ id: '3', name: 'Option Three', tags: ['green', 'third'] });
