...
describe('useMultiSelectList', () => {
  ...
  it('removeOption calls announce with type removed, label, and remaining count in multi mode', () => {
    props.value = ['1', '2', '3'];
    const announce = jest.fn();
    const { removeOption } = useMultiSelectList(props, emit, { searchText, announce });
    removeOption('2');
    expect(announce).toHaveBeenCalledWith({ type: 'removed', label: 'Option Two', count: 2 });
  });

  it('removeOption calls announce with type removed and no count in single mode', () => {
    props.multiple = false;
    props.value = '1';
    const announce = jest.fn();
    const { removeOption } = useMultiSelectList(props, emit, { searchText, announce });
    removeOption('1');
    expect(announce).toHaveBeenCalledWith({ type: 'removed' });
  });
  ...
  it('clearAll calls announce with type cleared', () => {
    props.value = ['1', '2'];
    const announce = jest.fn();
    const { clearAll } = useMultiSelectList(props, emit, { searchText, announce });
    clearAll();
    expect(announce).toHaveBeenCalledWith({ type: 'cleared' });
  });
