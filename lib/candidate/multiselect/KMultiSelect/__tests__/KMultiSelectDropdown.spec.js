...
describe('KMultiSelectDropdown — expanded mode', () => {
  it('renders in flow, without the floating panel modifier', () => {
    const { container } = renderDropdown({ expanded: true });
    const panel = container.querySelector('.kmselect-dropdown');
    expect(panel).toHaveClass('kmselect-dropdown-inline');
  });

  it('keeps the floating panel styling when expanded is off', () => {
    const { container } = renderDropdown();
    expect(container.querySelector('.kmselect-dropdown')).not.toHaveClass(
      'kmselect-dropdown-inline',
    );
  });

  it('applies the maxHeight prop to the list', () => {
    const { container } = renderDropdown({ expanded: true, maxHeight: '250px' });
    expect(container.querySelector('.kmselect-dropdown')).toHaveStyle({ maxHeight: '250px' });
  });

  it('marks the list inert when disabled in expanded mode', () => {
    const { container } = renderDropdown({ expanded: true, disabled: true });
    const panel = container.querySelector('.kmselect-dropdown');
    expect(panel).toHaveAttribute('aria-disabled', 'true');
    expect(panel).toHaveClass('is-disabled');
  });

  it('ignores disabled when expanded is off', () => {
    const { container } = renderDropdown({ disabled: true });
    expect(container.querySelector('.kmselect-dropdown')).not.toHaveAttribute('aria-disabled');
  });
});
