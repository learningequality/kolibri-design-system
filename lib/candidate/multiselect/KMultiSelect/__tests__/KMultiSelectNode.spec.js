...
describe('KMultiSelectNode', () => {
  it('renders a leaf node as a listbox option', async () => {
    ...
    expect(await screen.findByRole('option', { name: LEAF_NODE.label })).toBeInTheDocument();
  });

  it('renders a group node with its parent label and child label', async () => {
    renderComponent({ node: GROUP_NODE });
    expect(await screen.findByRole('option', { name: GROUP_NODE.label })).toBeInTheDocument();
    expect(
      await screen.findByRole('option', { name: GROUP_NODE.children[0].label }),
    ).toBeInTheDocument();
  });

  it('shows the indeterminate icon when a parent node is partially selected', async () => {
  ...
  it('applies paddingInlineStart indentation proportional to depth', async () => {
    ...
    const option = await screen.findByRole('option', { name: LEAF_NODE.label });
    expect(option).toHaveStyle({ paddingInlineStart: indentStyle(2) });
  });

  it('applies base 16px paddingInlineStart when depth is 0', async () => {
    renderComponent({ node: LEAF_NODE, depth: 0 });
    const option = await screen.findByRole('option', { name: LEAF_NODE.label });
    expect(option).toHaveStyle({ paddingInlineStart: indentStyle(0) });
  });
