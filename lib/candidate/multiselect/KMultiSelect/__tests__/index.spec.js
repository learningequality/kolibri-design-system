...
function renderKMultiSelect(propsOverrides = {}, options = {}) {
...
describe('KMultiSelect — expanded mode', () => {
  ...
  const renderExpanded = (propsOverrides = {}, options = {}) =>
    renderKMultiSelect({ expanded: true, ...propsOverrides }, options);
  ...
  it('filters the inline list as the user types', async () => {
    ...
    await waitFor(() => {
      ...
      expect(visible[0]).toHaveAccessibleName(OPTIONS[1].label);
    });
  });
  ...
  it('indents hierarchical options by depth in the inline list', async () => {
    ...
    const parent = await screen.findByRole('option', { name: HIERARCHICAL_OPTIONS[0].label });
    const child = await screen.findByRole('option', { name: HIERARCHICAL_OPTIONS[1].label });
  ...
  it('cascades a parent selection through the inline list with autoSelectChild', async () => {
    ...
    await userEvent.click(
      await screen.findByRole('option', { name: HIERARCHICAL_OPTIONS[0].label }),
    );
    const emitted = component.emitted()['input'];
    expect(emitted).toBeTruthy();
    expect(emitted[emitted.length - 1][0]).toEqual(
      HIERARCHICAL_OPTIONS.map(option => option.value),
    );
  });
