...
describe('KMultiSelect — showInvalidMessage guard', () => {
  it('does not show error on mount even when invalid=true', () => {
    renderKMultiSelect({ invalid: true, invalidText: 'Required' });
    expect(screen.queryByText('Required')).not.toBeInTheDocument();
    expect(getCombobox()).not.toHaveAttribute('aria-invalid', 'true');
  });

  it('shows error after the field is focused then blurred', async () => {
    renderKMultiSelect({ invalid: true, invalidText: 'Required' });
    getCombobox().focus();
    getCombobox().blur();
...
describe('KMultiSelect — required validation state', () => {
  it('shows error state after focus+blur when required and empty', async () => {
    renderKMultiSelect({ required: true, invalid: true, invalidText: 'Required', value: [] });
    getCombobox().focus();
    getCombobox().blur();
    await waitFor(() => expect(getCombobox()).toHaveAttribute('aria-invalid', 'true'));
    expect(screen.getByText('Required')).toBeInTheDocument();
  ...
  it('does not show error state when required but a value is selected', async () => {
    renderKMultiSelect({
      required: true,
      invalid: false,
