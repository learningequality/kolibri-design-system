...
describe('useMultiSelectCascade', () => {
  ...
  it('a promoted parent is ordered before its children, from parent to child', () => {
    const { onListboxInput } = makeCascade();
    props.value = onListboxInput(['child1']);
    const result = onListboxInput(['child1', 'child2']);
    expect(result).toEqual(['parent', 'child1', 'child2']);
  });

  it('with autoSelectParent, a directly selected parent with no selected children stays checked', () => {
  ...
  it('with autoSelectParent, ancestors are ordered before the child, from root to leaf', () => {
    normalizedOptions.value = [
      { value: 'grandparent', label: 'Grandparent', level: 1 },
      { value: 'parent', label: 'Parent', level: 2 },
      { value: 'child', label: 'Child', level: 3 },
    ...
    props.autoSelectParent = true;
    const { onListboxInput } = makeCascade();
    const result = onListboxInput(['child']);
  ...
  it('with autoSelectParent, selecting a child also selects its parent', () => {
    ...
    const { onListboxInput } = makeCascade();
    onListboxInput(['child1']);
    expect(emit).toHaveBeenCalledWith('input', ['parent', 'child1']);
  });
  ...
  it('with autoSelectParent, only the direct ancestor is selected, not a sibling-branch parent', () => {
    ...
    const { onListboxInput } = makeCascade();
    onListboxInput(['d']);
    // Should emit ['c', 'd'], not include 'a'
    expect(emit).toHaveBeenCalledWith('input', ['c', 'd']);
  });
