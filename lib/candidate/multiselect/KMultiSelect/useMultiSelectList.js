...
export default function useMultiSelectList(
  ...
  function isSelectedLabelQuery(query) {
    if (!props.expanded || props.multiple || !query) return false;
    ...
    return label != null && String(label).trim().toLowerCase() === query;
  }

  const displayedOptions = computed(() => {
    const isSuppressed = suppressFilter && suppressFilter.value;
    const rawQuery = !isSuppressed && searchText.value ? searchText.value.trim().toLowerCase() : '';
    const query = isSelectedLabelQuery(rawQuery) ? '' : rawQuery;
