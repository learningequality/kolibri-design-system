...
export default function useMultiSelectList(
  ...
  const displayedOptions = computed(() => {
    // When suppressFilter is true the dropdown was just opened and the input was
    // pre-filled with the current selection. We show ALL options in that state
    // so the user sees the full list (matching native <select> combobox UX).
    // suppressFilter is cleared the moment the user actually types something.
    const isSuppressed = suppressFilter && suppressFilter.value;
    const query = (!isSuppressed && searchText.value) ? searchText.value.toLowerCase() : '';
