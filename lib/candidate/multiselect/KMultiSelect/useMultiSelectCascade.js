...
export default function useMultiSelectCascade(
  ...
  function onListboxInput(newValues, { silent = false } = {}) {
    ...
    // A child was deselected: drop ancestors that are no longer fully covered.
    // Loops so demotion cascades upward.
    if (removed.length > 0) {
      ...
      while (demoted) {
        ...
        for (const ancestor of affectedAncestors) {
          ...
          // children, so it always demotes. Otherwise only auto-promoted
