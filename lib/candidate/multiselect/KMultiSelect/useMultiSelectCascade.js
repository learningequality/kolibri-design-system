...
export default function useMultiSelectCascade(
  props,
  emit,
  { normalizedOptions, getOptionValue, searchText, closeDropdown },
) {
  function getDescendants(value, flatOptions) {
    const idx = flatOptions.findIndex(o => getOptionValue(o) === value);
    if (idx === -1) return [];
  ...
  function onListboxInput(newValues) {
    if (!props.multiple) {
      emit('input', newValues.length > 0 ? newValues[newValues.length - 1] : null);
      if (searchText.value) {
        searchText.value = '';
      }
      closeDropdown();
