...
export default function useMultiSelectCascade(
  ...
  // Parents we checked automatically because the user had selected all their
  // children. If a child gets unchecked later, we uncheck these too, but a
  // parent the user checked themselves is left alone.
  ...
  function onListboxInput(newValues, { silent = false } = {}) {
    ...
    // A child was deselected: drop any auto-promoted ancestor that is no longer
    // fully covered (explicit ones stay). Loops so demotion cascades upward.
    if (removed.length > 0) {
      ...
      while (demoted) {
        ...
        for (const ancestor of affectedAncestors) {
          ...
          if (!derivedParents.has(ancestor)) continue;
