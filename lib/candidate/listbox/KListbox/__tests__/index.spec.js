...
describe('KListbox', () => {
  ...
  describe('disabled', () => {
    it('takes the list out of the tab order and marks it disabled', async () => {
    ...
    it('stays focusable and undisabled by default', async () => {
      renderComponent();
      const listbox = await screen.findByRole('listbox');
      expect(listbox).toHaveAttribute('tabindex', '0');
    ...
    it('can leave the tab order without being disabled', async () => {
      renderComponent({ props: { tabbable: false } });
      const listbox = await screen.findByRole('listbox');
      expect(listbox).toHaveAttribute('tabindex', '-1');
      expect(listbox).not.toHaveAttribute('aria-disabled');
    });

    it('ignores keyboard navigation and activation', async () => {
      const component = renderComponent({ props: { disabled: true } });
      const listbox = await screen.findByRole('listbox');
      listbox.focus();
    ...
    it('ignores the synthetic clicks assistive tech dispatches at an option', async () => {
      const component = renderComponent({ props: { disabled: true } });
      const option = await screen.findByRole('option', { name: OPTIONS[0].label });
      option.click();
      expect(component.emitted()['input']).toBeFalsy();
    ...
    const renderWithSelectAll = (props = {}) =>
      renderComponent({
        props: { multiple: true, ...props },
        scopedSlots: { selectAll: `<span slot-scope="s">All {{ s.allSelected }}</span>` },
      });

    const clickSelectAllRow = () => document.querySelector('.k-listbox-select-all').click();

    it('selects all from the select all row when not disabled', async () => {
      const component = renderWithSelectAll();
      clickSelectAllRow();
      expect(component.emitted()['input']).toBeTruthy();
    });

    it('ignores a click on the select all row when disabled', async () => {
      const component = renderWithSelectAll({ disabled: true });
      clickSelectAllRow();
      expect(component.emitted()['input']).toBeFalsy();
    });
  });

  describe('KListboxOption', () => {
    describe('selected option styling', () => {
      it('applies bold font-weight and visible checkmark to the selected option label', async () => {
        renderComponent({ props: { multiple: false, value: [OPTIONS[0].id] } });
        const option = await screen.findByRole('option', { name: OPTIONS[0].label });

        const labelSpan = option.querySelector('.k-option-plain-label');
        expect(labelSpan).toHaveStyle('font-weight: 500');

        const checkIcon = option.querySelector('.k-option-check-icon');
        expect(checkIcon).toHaveStyle('visibility: visible');
      });
    });
  });
});
