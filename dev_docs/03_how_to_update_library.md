...
Learning Equality products are in the process of transitioning their package managers from `yarn` (v1) to `pnpm`. Kolibri Studio has completed this transition. You can use either `yarn link` or `pnpm link` depending on the product's package manager, although the process using `pnpm` is simpler. First, determine which package manager is used in the product (e.g. `yarn` or `pnpm`)

#### For `yarn`:

1. Ensure that `yarn` is installed and available in the `kolibri-design-system` project.
2. While in the root of your local `kolibri-design-system` repository, link the package: `yarn link`.
3. In the root of a product where you intend to use `kolibri-design-system` run `yarn link kolibri-design-system` and then `yarn install`.

#### For `pnpm`:
1. Ensure that `pnpm` is installed and available in your project directory of the product where you intend to use `kolibri-design-system`.
2. In the same project directory, link `kolibri-design-system` with its path `pnpm link ../kolibri-design-system` (in this case, the project directories are siblings) and then `pnpm install`.
